...
serve(async (req) => {
  ...
  try {
    ...
    const { word, source } = await req.json() as {
      word: {
        headword: string;
        synonyms?: string[];       // frontend field name (DetailCardData)
        nativeSynonyms?: string[]; // alternative field name (generated by card functions)
    ...
    if (!word?.headword || !word?.contexts || !source) {
